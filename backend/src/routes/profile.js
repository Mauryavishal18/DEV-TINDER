...
// PATCH /profile/edit
const ALLOWED = ["firstName","lastName","photoUrl","about","gender","age","skills","github","portfolio","linkedin","leetcode","gfg","useAvatar","avatarSeed"];
...
router.post("/profile/upload-photo", userAuth, upload.single("photo"), async (req, res) => {
  try {
    ...
    req.user.useAvatar = false;
    await req.user.save();
    const u = req.user.toObject(); delete u.password;
    res.json({ message: "Photo uploaded!", user: u, photoUrl });
  } catch (err) {
    res.status(400).json({ message: err.message });
  }
});
