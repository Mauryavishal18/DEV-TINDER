...
export default function LoginPage() {
  ...
  const login = async () => {
    ...
    if (captchaInput.trim() !== captcha) { setError("Incorrect CAPTCHA. Try again."); setCaptcha(generateCaptcha()); setCaptchaInput(""); return; }
    try {
      setLoading(true); setError("");
      const r = await axios.post("/login", { emailId: email, password: pass });
      dispatch(addUser(r.data.user || r.data));
      navigate("/");
    } catch(e) { setError(e.response?.data?.message || "Invalid credentials"); setCaptcha(generateCaptcha()); setCaptchaInput(""); }
